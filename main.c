...
void handle_client_connection(int client_socket_fd, char *backend_host, char* backend_port_str) {
	...
	getaddrinfo = getaddrinfo(backend_host, backend_port_str, &hints, &addrs);
	...
	for (addrs_iter = addrs; addrs_iter != NULL; addr_iter = addr_iter->ai_next) {
	...
	if (addrs_iter == NULL) {
		fprintf(stderr, "could not connect to the backend\n");
		exit(1);
	}
